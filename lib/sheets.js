...
async function savePluggyAccounts(accounts) {
  const rows = accounts.map((account) => [
    ...
    Number(account.balance) || 0, account.creditData?.brand || '', account.creditData?.balanceCloseDate || '', account.creditData?.balanceDueDate || '',
...
async function listPluggyAccounts() {
  ...
  return (await getRange('PluggyContas!A2:P')).filter((row) => row[0]).map((row) => ({
    ...
    balance: Number(row[6]) || 0, currencyCode: String(row[7] || 'BRL'), creditLimit: Number(row[8]) || 0, availableLimit: Number(row[9]) || 0,
    usedLimit: Number(row[10]) || 0, brand: String(row[11] || ''), closeDate: String(row[12] || ''), dueDate: String(row[13] || ''), status: String(row[14] || ''), updatedAt: String(row[15] || '')
  }));
}
...
async function listPluggyTransactions(month) {
  ...
  return (await getRange('PluggyTransacoes!A2:O')).filter((row) => row[0] && (!month || String(row[6]).startsWith(month))).map((row) => ({
    ...
    valor: Number(row[5]) || 0, data: String(row[6]), dataPg: String(row[7]) === 'Pago' ? String(row[6]) : '', mesRef: String(row[6]).slice(0, 7), status: String(row[7]),
...
module.exports = {
  ...
  registerPluggyEvent, hasPluggyEvent, deletePluggyTransactions
};
