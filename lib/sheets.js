...
async function listPluggyBills(month) {
  await ensurePluggyStorage();
  const bills = (await getRange('PluggyFaturas!A2:N')).filter((row) => row[0] && (!month || String(row[4]).startsWith(month))).map((row) => {
    ...
    return {
      ...
      descricao: `Fatura ${String(row[3])}`, valor: hasLocalValue ? parseMoney(row[12]) : parseMoney(row[6]), data: date, dataPg: status === 'Pago' ? date : '', mesRef: date.slice(0, 7),
      status, tipo: 'Pagamento Fatura', categoria: 'Fatura de cartão', responsavel: 'Nós', parcelaAtual: 1, totalParcelas: 1, idParcelamento: '',
      recorrente: false, fonte: 'pluggy-bill', estimada: String(row[10]).toLowerCase() === 'true', ajustada: hasLocalValue,
...
module.exports = {
  ...
  savePluggyBills, listPluggyBills, updatePluggyBillStatus, updatePluggyBill, registerPluggyEvent, hasPluggyEvent, deletePluggyTransactions, deletePluggyItem
};
