...
const { normalizeTransaction, parseMoney } = require('./finance');
...
const PLUGGY_SHEETS = {
  ...
  PluggyContas: ['ID', 'ItemID', 'Tipo', 'Subtipo', 'Nome', 'Numero', 'Saldo', 'Moeda', 'LimiteTotal', 'LimiteDisponivel', 'LimiteUsado', 'Bandeira', 'Fechamento', 'Vencimento', 'Status', 'AtualizadoEm'],
  ...
  PluggyFaturas: ['ID', 'ItemID', 'ContaID', 'Conta', 'Vencimento', 'Fechamento', 'Valor', 'Pagamentos', 'StatusProvedor', 'StatusLocal', 'Estimada', 'AtualizadoEm'],
  PluggyEventos: ['ID', 'Evento', 'RecebidoEm']
};
...
async function listPluggyAccounts() {
  await ensurePluggyStorage();
  return (await getRange('PluggyContas!A2:P')).filter((row) => row[0]).map((row) => ({
...
async function listPluggyTransactions(month) {
  await ensurePluggyStorage();
  return (await getRange('PluggyTransacoes!A2:O')).filter((row) => row[0] && (!month || String(row[6]).startsWith(month))).map((row) => ({
...
async function listPluggyBills(month) {
  await ensurePluggyStorage();
  const bills = (await getRange('PluggyFaturas!A2:L')).filter((row) => row[0] && (!month || String(row[4]).startsWith(month))).map((row) => {
    const status = String(row[9] || row[8] || 'Pendente');
    ...
    return {
      ...
      descricao: `Fatura ${String(row[3])}`, valor: parseMoney(row[6]), data: date, dataPg: status === 'Pago' ? date : '', mesRef: date.slice(0, 7),
      status, tipo: 'Despesa', categoria: 'Fatura de cartão', responsavel: 'Nós', parcelaAtual: 1, totalParcelas: 1, idParcelamento: '',
      recorrente: false, fonte: 'pluggy-bill', estimada: String(row[10]).toLowerCase() === 'true'
    };
  });
  ...
  for (const bill of bills) {
    ...
    if (!current || (current.estimada && !bill.estimada)) unique.set(key, bill);
  }
...
async function registerPluggyEvent(eventId, eventName) {
  ...
  if (rows.some((row) => row[0] === eventId)) return false;
  await upsertRows('PluggyEventos', [[eventId, eventName, new Date().toISOString()]]);
  return true;
}
...
module.exports = {
  ...
  savePluggyItem, listPluggyItems, savePluggyAccounts, listPluggyAccounts, savePluggyTransactions, listPluggyTransactions,
  savePluggyBills, listPluggyBills, updatePluggyBillStatus, registerPluggyEvent, hasPluggyEvent, deletePluggyTransactions, deletePluggyItem
};
