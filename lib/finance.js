...
const TYPES = ['Receita', 'Despesa', 'Depósito', 'Depósito Caixinha', 'Resgate'];
...
function summarize(transactions, boxes = [], boxTransactions = transactions) {
  ...
  for (const item of transactions) {
    ...
    const inCurrentAccount = item.origem === CURRENT_ACCOUNT || item.origem === 'Cartão de Crédito';
    if (inCurrentAccount && item.tipo === 'Receita') summary.saldoPrevisto += item.valor;
    if (inCurrentAccount && item.tipo === 'Despesa') summary.saldoPrevisto -= item.valor;
    if (inCurrentAccount && isDeposit) summary.saldoPrevisto -= item.valor;
    if (inCurrentAccount && item.tipo === 'Resgate') summary.saldoPrevisto += item.valor;
    if (inCurrentAccount && item.status === 'Pago' && item.tipo === 'Receita') summary.saldoAtual += item.valor;
    if (inCurrentAccount && item.status === 'Pago' && item.tipo === 'Despesa') summary.saldoAtual -= item.valor;
    if (inCurrentAccount && item.status === 'Pago' && isDeposit) summary.saldoAtual -= item.valor;
    if (inCurrentAccount && item.status === 'Pago' && item.tipo === 'Resgate') summary.saldoAtual += item.valor;
  }
...
function accountEffect(item) {
  ...
  if (item.tipo === 'Despesa' || item.tipo === 'Depósito' || item.tipo === 'Depósito Caixinha') return -item.valor;
  return 0;
}
...
module.exports = { normalizeTransaction, toSheetRow, validateTransaction, summarize, calculateCashFlow, buildAnalysis, parseMoney };
